...
def foo():
    ...
    df['winloss_zszy'] = np.where(df.nll2 < df.止损价, -(df.atr*n+费用), df.winloss_zy)  # 止损 止盈 都有 看哪个比例好   先计止损
    #df['winloss_zszy'] = np.where(df.nll2 < df.止损价, -(df.atr+费用), df.winloss_zy) # 先计止盈    结果这个应该比前者好， 但这是bug，真实结果介于两者之间
    ...
    #df['condition6'] = np.where(df.c > df.c.shift(2) , 1, None)  # 前仓浮盈， 才能开后仓  df.c 这次开仓价   df.c.shift(n) 代表上次开仓价
    ...
    '''
    (df.ma1.shift(1) > df.ma2.shift(1))  &
    (df.ma2.shift(1)  > df.ma3.shift(1)) &
    (df.ma3.shift(1)  > df.ma4.shift(1))
    '''
